(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["bitbucketAuth"] = factory();
	else
		root["bitbucketAuth"] = factory();
})(this, function() {
...
var _require = __webpack_require__(5),
    ...
    stringProps = _require.stringProps;

var defaults = {
  domain: 'bitbucket.org',
  errorHandler: function errorHandler(method) {
...
var errorHandler = defaults.errorHandler;
...
var secretKeys = ['consumerKey', 'consumerSecret'];
...
var _require = __webpack_require__(3),
    getAccessToken = _require.getAccessToken;
...
var _require = __webpack_require__(0),
    ...
    populateDefaults = _require.populateDefaults,
    setErrorHandler = _require.setErrorHandler;

var _require2 = __webpack_require__(6),
    sendRequest = _require2.sendRequest,
    createRequestHandler = _require2.createRequestHandler;
...
function getAccessToken() {
  ...
  var validate = createValidator('getAccessToken');
  validate(opts);

  var _opts = opts,
      loadConfig = _opts.loadConfig,
      storage = _opts.storage,
      configPath = _opts.configPath;

  storage = storage || {};
  var logger = opts.logger || defaults.logger;
  var log = logger('getAccessToken', opts);

  var loadFromStorage = loadConfig || storage.loadConfig ? storage.loadConfig.bind(storage) : null;
  var config = loadFromStorage ? loadFromStorage(opts) : {};
  opts = Object.assign({}, {
...
function getTokens() {
  ...
  var payload = void 0;
  var errorMessageOn401 = 'getTokens: Authentication failed!';
  ...
  var errorHandler = setErrorHandler('getTokens', opts);
  var logger = opts.logger || defaults.logger;
  var log = logger('getTokens', opts);
  ...
  if (username) {
    ...
    log('prepared for Basic auth', {
      payload: payload,
      errorMessageOn401: errorMessageOn401
    });
...
function useRefreshToken(config) {
  var refreshToken = config.refreshToken,
      forceCredentials = config.forceCredentials;
...
var required = ['appName'];

var stringProps = ['appName', 'consumerKey', 'consumerSecret', 'refreshToken', 'configPath'];
...
function sendRequest() {
  ...
  var log = (logger || _util.defaults.logger)('sendRequest', opts);
  ...
  return new Promise(function (resolve, reject) {
    ...
    var uri = 'https://' + domain + '/site/oauth2/access_token';
    var handlerOpts = Object.assign(opts, {
      config: config,
      resolve: resolve,
      reject: reject
    });
    ...
    var requestHandler = (opts.createRequestHandler || createRequestHandler)(handlerOpts);
    request.post(uri).auth(consumerKey, consumerSecret).accept('application/json').type('form').send(payload).end(requestHandler);
  });
}
...
function createRequestHandler() {
  ...
  var config = opts.config,
      ...
      reject = opts.reject,
      errorMessageOn401 = opts.errorMessageOn401,
      logger = opts.logger;

  config = config || {};
  errorMessageOn401 = errorMessageOn401 || 'ERROR:';
  var log = (logger || _util.defaults.logger)('requestHandler', opts);
...
function handleError() {
  ...
  var log = (logger || _util.defaults.logger)('handleError', opts);
...
function handleSuccess() {
  ...
  storage = storage || {};
  var log = (logger || _util.defaults.logger)('handleSuccess', opts);
  var refresh_token = body.refresh_token,
      access_token = body.access_token,
      expires_in = body.expires_in,
      token_type = body.token_type;

  var accessResponsePayload = {
  ...
  var saveToStorage = saveConfig || storage.saveConfig ? storage.saveConfig.bind(storage) : null;
...
//# sourceMappingURL=bitbucketAuth.js.map